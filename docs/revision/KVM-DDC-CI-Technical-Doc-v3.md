...
> 最后更新：2026-04-05
> 阶段：LG 显示器 + USB 切换已实现，DDC 读取调试中，小米显示器等待硬件
...
| DDC/CI 读取轮询 | 🔧 调试中 | 读取失败，诊断版已部署 |
| OSD 切换自动同步 USB | 🔧 依赖读取 | 读取修好后即可工作 |
...
写入 100% 成功，但读取持续失败。诊断版代码尝试三种方式：

| 方法 | 策略 | 状态 |
|------|------|------|
| A | transmit → 80ms delay → receive | 待确认 |
| B | transmit_receive (repeated start) | 待确认 |
| C | transmit → 150ms delay → receive (长延迟) | 待确认 |

**可能原因：**
- GPU 周期性占用 DDC 总线做 EDID 查询，ESP32 receive 被 NACK
- 显示器对 VCP 0x60 的读取回复需要更长准备时间
- 回复数据有偏移或前缀（NULL 字节等）
...
| 症状 | 可能原因 | 诊断方案 |
|------|---------|---------|
| DDC 读取 VCP 0x60 持续失败 | GPU 占用总线 / 时序问题 / 回复格式偏移 | 诊断版代码已部署，尝试三种读取方式 |
...
1. ~~部署 DDC 读取诊断版~~ ✅ 已部署
2. 分析诊断日志，确定读取失败原因
3. 修复 DDC 读取，实现 OSD 自动同步

### 12.2 中期（等待硬件到货）

4. 收到分线板 + 母对母后，验证 Redmi 串联方案
5. 确认 Redmi HDMI1 的 VCP 0x60 值
6. 实现双显示器同步切换
7. 添加物理按钮（GPIO 触发切换）

### 12.3 远期

8. 所有功能面包板验证通过后，设计 PCB
9. 嘉立创打样 + SMT 贴片
10. 整理进成品外壳

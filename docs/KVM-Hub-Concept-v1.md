...
> **⚠️ I2C-0 总线地址冲突风险：** LG 和 Redmi HDMI1 共享 I2C-0 总线，两台显示器的 DDC 从机地址均为 0x37，会同时响应造成总线冲突。解决方案：在 LG 的 DDC 支路上增加一个 I2C 总线开关（如 TCA9548A 的单通道，或用 GPIO 控制的模拟开关 TCA9406 / 低成本方案用 MOSFET 门控 SDA/SCL），ESP32 在访问 LG 时打开该支路、关闭 Redmi 支路，反之亦然。或者更简单：将 LG DDC 单独分配到 I2C-1，Redmi HDMI2 DDC 分配到 I2C-0，这样三路 DDC 变为两条总线各挂一个设备，彻底避免冲突（需调整 GPIO 分配）。
...
> **⚠️ USB 热插拔行为：** TS3USB221 切换上行主机时，对 PC 和 Mac 而言等同于 USB 集线器物理拔插。目标主机需要重新枚举所有下游设备，键盘/鼠标会有 1-2 秒的短暂断连。这是模拟开关方案的固有行为，无法避免，但对日常使用影响很小。固件中应在切换前后加入适当延时（~100ms），确保信号稳定后再让主机开始枚举。
...
> **⚠️ 未使用端口处理：** Root hub 的 2 个空闲下行端口在 PCB 上不应引出连接器或走长线。建议将未使用端口的 D+/D- 各串接一个 15kΩ 下拉电阻到 GND（符合 USB 规范的未连接端口处理），避免悬空线充当天线引入 EMI 噪声，影响相邻 TMDS 差分对的信号完整性。
...
> **⚠️ DDC T 型分接高阻要求：** ESP32 的 I2C GPIO 在不进行 DDC/CI 通信时，必须配置为高阻输入模式（input with no pull-up），避免干扰显卡与显示器之间的 EDID 握手。如果 ESP32 在错误时刻驱动 SDA/SCL 线，可能导致显示器无法被识别或 EDID 读取失败。固件实现上，建议在 DDC/CI 操作前动态初始化 I2C 外设，操作完成后立即释放并将 GPIO 恢复为高阻状态。
...
> **⚠️ USB 端口限流保护：** 8 个 USB-A 口按 USB 2.0 规范每口最大 500mA，理论峰值可达 4A，远超 5V/3A 供电能力。虽然当前外设实际总功耗约 1.5A，但如果用户误插入大电流设备（如移动硬盘、手机充电），可能导致整板电压跌落甚至损坏。建议每个 USB-A 口或每组 4 口加限流保护芯片（如 SY6280AAC，SOT-23-5 封装，可配置限流阈值 500mA），同时提供过流指示引脚可接 ESP32 GPIO 监控。
...
| 表面处理 | HASL 或沉金 |
| 最小线宽/间距 | 5mil / 5mil |
| 尺寸 | 预估 150mm x 100mm（见下方说明） |

> **⚠️ 板子尺寸调整：** 原估 120x80mm 偏紧。后面板需容纳 5 个 HDMI 母座（每个约 15mm 宽）+ 2 个 USB 上行口 + 1 个 USB-C 供电口，仅连接器排列就需约 110mm 宽度加间距。前面板 8 个 USB-A（每个约 14mm 宽）也需要 ~130mm。建议初始尺寸调整为 150x100mm，PCB layout 阶段根据实际布局再优化。如果面积受限，可考虑 USB-A 口分两排上下错开排列。
...
| **v1.1** | **2026-04-05** | 补充设计注意事项：I2C-0 总线地址冲突风险及解决方案、USB 热插拔行为说明、DDC T 型分接高阻要求、未使用 hub 端口处理、USB 端口限流保护、PCB 尺寸调整 |
